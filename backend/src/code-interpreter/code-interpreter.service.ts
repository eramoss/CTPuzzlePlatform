...
export class CodeInterpreterService {
  ...
  execute(script: string, rejectOnError = false): Promise<string> {
    return this.executeAsync(script, rejectOnError);
  }

  //deprecated
  executeSync(script: string, rejectOnError: boolean): Promise<string> {
    const path = __dirname + '/test.ts';
    fs.writeFileSync(path, script);
    const deno: SpawnSyncReturns<Buffer> = spawnSync(
      this.denoLocation,
      ['run', path],
      { env: { NO_COLOR: 'true' }, timeout: 100 },
    );
    const stderr = deno.stderr.toString();
    if (rejectOnError) {
      if (stderr) {
        Promise.reject(stderr);
      }
    }
    const result = this.removeUndesiredLines(deno.output.toString());
    return Promise.resolve(result);
  }

  executeAsync(script: string, rejectOnError: boolean): Promise<string> {
    return new Promise((resolve, reject) => {
      const deno = spawn(this.denoLocation, ['run', '-'], {
        env: { NO_COLOR: 'true' },
      });
      ...
      deno.on('close', (code) => {
        ...
        const result = this.removeUndesiredLines(lines);
        resolve(result);
      });
